-- Simple fix for trade functions - just fix the parameter name issue
-- Don't break what's working!

-- Ensure ships table has credits column (in case migration 138 wasn't run)
...
-- Restore the original working functions with just the parameter name fix
...
BEGIN
  ...
  CASE p_resource
    WHEN 'ore' THEN v_unit_price := v_port.price_ore;
    WHEN 'organics' THEN v_unit_price := v_port.price_organics;
    WHEN 'goods' THEN v_unit_price := v_port.price_goods;
    WHEN 'energy' THEN v_unit_price := v_port.price_energy;
  END CASE;
  ...
  IF p_action = 'buy' THEN
    ...
    IF p_qty > v_cargo_free THEN
      RETURN json_build_object('error', json_build_object('code','insufficient_cargo_space','message','Insufficient cargo space'));
    END IF;
...
-- Restore the original working game_trade_auto function with just the parameter name fix
...
BEGIN
  ...
  buy_prices := jsonb_build_object(
    'ore', v_port.price_ore * 1.10,
    'organics', v_port.price_organics * 1.10,
    'goods', v_port.price_goods * 1.10,
    'energy', v_port.price_energy * 1.10
  );
