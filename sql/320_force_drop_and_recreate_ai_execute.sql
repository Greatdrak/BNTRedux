-- Migration: 320_force_drop_and_recreate_ai_execute.sql
-- Purpose: Force drop all versions and recreate ai_execute_action function

-- Drop ALL versions of the function (PostgreSQL allows multiple overloads)
DROP FUNCTION IF EXISTS public.ai_execute_action(uuid, uuid, text);
DROP FUNCTION IF EXISTS public.ai_execute_action(uuid, text);
DROP FUNCTION IF EXISTS public.ai_execute_action(text, uuid, text);
DROP FUNCTION IF EXISTS public.ai_execute_action(text, text, text);
...
BEGIN
  ...
  PERFORM log_ai_action(
    ...
    'action_attempt',
    'info',
    'Attempting action: ' || p_action || ' for ' || v_player_handle
  );
  ...
  CASE p_action
      ...
      IF v_planet_id IS NOT NULL THEN
        BEGIN
          ...
          BEGIN
            ...
            -- Use v_user_id for game_planet_claim
        ...
        IF v_sector_id IS NOT NULL THEN
          ...
          IF v_sector_number IS NOT NULL THEN
            ...
            BEGIN
              ...
              -- Check for 'ok' field in game_move response
    ...
    ELSE
      ...
      PERFORM log_ai_action(
        ...
        'unknown_action',
        'error',
        'Unknown action: ' || p_action || ' for ' || v_player_handle
      );
      v_result := false;
  END CASE;
