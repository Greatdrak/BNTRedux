-- Fix port stock caps to match new base stock amounts
...
-- Create the corrected function with proper caps
...
DECLARE
  ...
  ore_cap bigint := 100000000;        -- 100M cap for ore
  organics_cap bigint := 100000000;   -- 100M cap for organics  
  goods_cap bigint := 100000000;      -- 100M cap for goods
  energy_cap bigint := 1000000000;    -- 1B cap for energy
BEGIN
  /*
  ...
  */
  ...
  updated AS (
    UPDATE public.ports p SET
      ore = CASE
        WHEN p.kind = 'ore' THEN LEAST(ore_cap, p.ore + FLOOR((ore_cap - p.ore) * regen_rate)::bigint)
        ELSE GREATEST(0, p.ore - FLOOR(p.ore * decay_rate)::bigint)
      END,
      organics = CASE
        WHEN p.kind = 'organics' THEN LEAST(organics_cap, p.organics + FLOOR((organics_cap - p.organics) * regen_rate)::bigint)
        ELSE GREATEST(0, p.organics - FLOOR(p.organics * decay_rate)::bigint)
      END,
      goods = CASE
        WHEN p.kind = 'goods' THEN LEAST(goods_cap, p.goods + FLOOR((goods_cap - p.goods) * regen_rate)::bigint)
        ELSE GREATEST(0, p.goods - FLOOR(p.goods * decay_rate)::bigint)
      END,
      energy = CASE
        WHEN p.kind = 'energy' THEN LEAST(energy_cap, p.energy + FLOOR((energy_cap - p.energy) * regen_rate)::bigint)
