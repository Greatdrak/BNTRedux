-- Migration: 252_completely_fix_create_universe.sql
-- Completely fix the create_universe function by dropping all versions and recreating the correct one

-- Drop ALL versions of create_universe function
...
DROP FUNCTION IF EXISTS public.create_universe CASCADE;

-- Now create the ONLY correct version of create_universe
...
BEGIN
  ...
  -- TEMPORARILY DISABLE the warp limit trigger during universe creation
  ...
  -- Get current warp count after backbone creation
...
EXCEPTION
  WHEN OTHERS THEN
    ...
    EXCEPTION
      WHEN OTHERS THEN
        -- Ignore trigger recreation errors during cleanup
        NULL;
    END;
