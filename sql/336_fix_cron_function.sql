-- Update cron function to use the working simple debug function
-- This will fix the AI processing issue
...
BEGIN
  ...
  BEGIN
    -- Use the working simple debug function
    v_raw := COALESCE(
      (SELECT to_jsonb(x) FROM (SELECT * FROM simple_ai_debug(p_universe_id)) x),
    ...
    v_result := jsonb_build_object(
      ...
      'ai_total', COALESCE((v_raw->>'total_ai')::int, 0),
      'ai_with_turns', COALESCE((v_raw->>'ai_with_turns')::int, 0),
      'ai_with_goal', COALESCE((v_raw->>'total_ai')::int, 0)
    );
