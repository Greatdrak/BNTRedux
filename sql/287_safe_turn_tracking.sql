-- Migration: 287_safe_turn_tracking.sql
-- Make turn tracking optional/safe in game_move to avoid runtime errors if helper is missing
...
BEGIN
  ...
  -- Track a turn spent for leaderboard/activity (safe if helper missing)
  BEGIN
    PERFORM public.track_turn_spent(v_player_id);
  EXCEPTION WHEN undefined_function THEN
    -- ignore if tracking helper not installed
    NULL;
  END;
